...
export const FractionReducerSettings: React.FC = () => {
  ...
  useEffect(() => {
    localStorage.setItem('fractionreducer_settings', JSON.stringify(settings));
  }, [settings]);
  ...
  return (
    <div className="max-w-lg mx-auto p-4">
      <h2 className="text-2xl font-bold mb-6 text-center">
        {settings.language === 'english' ? 'Fraction Reducer Settings' : 'Configuración de Reducción de Fracciones'}
      </h2>
      <div className="space-y-6">
        ...
        <div className="flex items-center">
          <input
            type="checkbox"
            id="includeNonFactors"
            name="includeNonFactors"
            checked={settings.includeNonFactors}
            onChange={handleChange}
            className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
          />
          <label htmlFor="includeNonFactors" className="ml-2 block text-sm text-gray-700 dark:text-gray-300">
...
export default FractionReducerSettings;