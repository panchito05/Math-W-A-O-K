...
import { generateProblem, checkAnswer, simplifyMixedNumber } from './utils';
...
export const ImproperToMixedExercise: React.FC = () => {
  // Emit progress event
  ...
  const [settings, setSettings] = useState<Settings>(() => {
    const saved = localStorage.getItem('impropertomixed_settings');
  ...
  useEffect(() => {
    const newProblem = generateProblem(settings.difficulty, settings.requireSimplifiedForm);
    setCurrentProblem(newProblem);
    startTime.current = Date.now();
  }, [settings.difficulty, settings.requireSimplifiedForm]);
  ...
  const handleContinue = () => {
    ...
    if (currentIndex < targetProblemCount - 1) {
      ...
      setCurrentProblem(generateProblem(settings.difficulty, settings.requireSimplifiedForm));
      setWhole('');
      setNumerator('');
      setDenominator('');
      setShowSteps(false);
      ...
      if (wholeInputRef.current) wholeInputRef.current.focus();
    } else {
      setIsComplete(true);
    }
  };
  ...
  const calculateIntermediateResult = (problem: Problem) => {
    const whole = Math.floor(problem.numerator / problem.denominator);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isNaN(wholeValue) || isNaN(numValue) || isNaN(denValue) || denValue === 0) {
      toast.error(settings.language === 'english' 
        ? 'Please enter valid numbers. Denominator cannot be zero.' 
        : 'Por favor, ingrese números válidos. El denominador no puede ser cero.');
      return;
    }
    ...
    if (correct) {
      ...
      if (hasReachedMaxAttempts) {
        toast.error(settings.language === 'english' 
          ? `Incorrect. The correct answer was ${currentProblem.correctAnswer.whole} ${
              currentProblem.correctAnswer.numerator
            }/${currentProblem.correctAnswer.denominator}` 
          : `Incorrecto. La respuesta correcta era ${currentProblem.correctAnswer.whole} ${
              currentProblem.correctAnswer.numerator
            }/${currentProblem.correctAnswer.denominator}`);
          ...
          <div className="mb-6">
            ...
            <div className="text-4xl font-bold flex items-center justify-center">
              <div className="flex flex-col items-center">
                <span className="text-yellow-600 dark:text-yellow-400">{reviewAnswer.problem.numerator}</span>
                <div className="w-8 h-0.5 bg-current my-1"></div>
                <span className="text-cyan-600 dark:text-cyan-400">{reviewAnswer.problem.denominator}</span>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            {reviewAnswer.answer ? (
              <div className={`text-4xl font-bold flex items-center justify-center ${
                ...
                <span className="mr-2">{reviewAnswer.answer.whole}</span>
                <div className="flex flex-col items-center">
          ...
          {(!reviewAnswer.isCorrect || reviewAnswer.wasRevealed) && (
            <div className="mb-6">
              ...
              <div className="text-4xl font-bold flex items-center justify-center text-green-600 dark:text-green-400">
                <span className="mr-2">{reviewAnswer.problem.correctAnswer.whole}</span>
                {reviewAnswer.problem.correctAnswer.numerator > 0 && (
                  <div className="flex flex-col items-center">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg mb-6">
        <div className="text-center mb-6">
          ...
          <div className="mb-6">
            ...
            <div className="text-4xl font-bold flex items-center justify-center">
              <div className="flex flex-col items-center">
                <span className="text-yellow-600 dark:text-yellow-400">{currentProblem.numerator}</span>
                <div className="w-8 h-0.5 bg-current my-1"></div>
                <span className="text-cyan-600 dark:text-cyan-400">{currentProblem.denominator}</span>
              </div>
            </div>
          </div>
                ...
                <>
                  <div>
                    <p className="text-xl text-pink-600 dark:text-pink-400 mb-2">
                      {settings.language === 'english'
                        ? `Divide the numerator by the denominator: ${currentProblem.numerator} ÷ ${currentProblem.denominator}`
                        : `Divide el numerador entre el denominador: ${currentProblem.numerator} ÷ ${currentProblem.denominator}`
                      }
                    </p>
                    <div className="text-2xl font-bold flex items-center justify-center space-x-2">
                      ...
                      <span className="text-cyan-600 dark:text-cyan-400">{currentProblem.denominator}</span>
                      <span className="text-gray-600 dark:text-gray-400">=</span>
                      <span className="text-green-600 dark:text-green-400">{intermediateResult.whole}</span>
                      {intermediateResult.remainder > 0 && (
                        <>
                          <span className="text-gray-600 dark:text-gray-400">with remainder</span>
                          <span className="text-orange-600 dark:text-orange-400">{intermediateResult.remainder}</span>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    <p className="text-xl text-pink-600 dark:text-pink-400 mb-2">
                      {settings.language === 'english'
                        ? `The whole number part is ${intermediateResult.whole}`
                        : `La parte entera es ${intermediateResult.whole}`
                      }
                    </p>
                    {intermediateResult.remainder > 0 ? (
                      <p className="text-xl text-pink-600 dark:text-pink-400 mb-2">
                        {settings.language === 'english'
                          ? `The fractional part is ${intermediateResult.remainder}/${currentProblem.denominator}`
                          : `La parte fraccionaria es ${intermediateResult.remainder}/${currentProblem.denominator}`
                        }
                      </p>
                    ) : (
                      <p className="text-xl text-pink-600 dark:text-pink-400 mb-2">
                        {settings.language === 'english'
                          ? 'There is no fractional part (remainder is 0)'
                          : 'No hay parte fraccionaria (el residuo es 0)'
                        }
                      </p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="text-2xl font-bold flex items-center justify-center">
                      <span className="text-green-600 dark:text-green-400 mr-2">{currentProblem.correctAnswer.whole}</span>
                      {currentProblem.correctAnswer.numerator > 0 && (
                        <div className="flex flex-col items-center">
                          <span className="text-orange-600 dark:text-orange-400">{currentProblem.correctAnswer.numerator}</span>
                          <div className="w-8 h-0.5 bg-current my-1"></div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="flex items-center justify-center space-x-4">
            <input
              ...
              className="w-16 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-center bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
              placeholder={settings.language === 'english' ? 'Whole' : 'Entero'}
              aria-label={settings.language === 'english' ? 'Whole number' : 'Número entero'}
            ...
            <div className="flex flex-col items-center">
              <input
                ...
                disabled={isAnswerRevealed || hasReachedMaxAttempts}
              />
              <div className="w-16 h-0.5 bg-gray-400 dark:bg-gray-500 my-1"></div>
              <input
                type="number"
                value={denominator}
                min="1"
                ...
                {isCorrect 
                  ...
                  : (settings.language === 'english' 
                      ? `Incorrect. ${settings.maxAttempts > 0 && currentAttempts >= settings.maxAttempts 
                          ? `The correct answer is ${currentProblem.correctAnswer.whole} ${
                              currentProblem.correctAnswer.numerator > 0 
                      ...
                      : `Incorrecto. ${settings.maxAttempts > 0 && currentAttempts >= settings.maxAttempts 
                          ? `La respuesta correcta es ${currentProblem.correctAnswer.whole} ${
                              currentProblem.correctAnswer.numerator > 0 
                                ? `${currentProblem.correctAnswer.numerator}/${currentProblem.correctAnswer.denominator}` 
                                : ''
                            }` 
...
export default ImproperToMixedExercise;