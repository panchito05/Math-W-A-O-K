...
// NUEVO: Probabilidad de que ambos números sean del rango principal del nivel
const SAME_RANGE_PROBABILITY = 0.6; // 60% de probabilidad de N-dígitos + N-dígitos
...
// Helper para obtener entero aleatorio (reutilizado internamente)
const getRandomInt = (minNum: number, maxNum: number): number => {
    minNum = Math.ceil(minNum);
    maxNum = Math.floor(maxNum);
    // Asegurar que maxNum >= minNum para evitar errores de rango
    if (maxNum < minNum) {
        [minNum, maxNum] = [maxNum, minNum]; // Intercambiar si están invertidos
    }
    return Math.floor(Math.random() * (maxNum - minNum + 1)) + minNum;
}

// --- MODIFICADO: generateProblem con rangos variables ---
const generateProblem = (difficulty: number): Problem => {
    let min: number, max: number;
    let lowerMin: number = 1, lowerMax: number = 9; // Rango inferior por defecto (para L2)
    const validDifficulty = Math.max(1, Math.min(MAX_DIFFICULTY, Math.round(difficulty)));

    // Definir rangos principal e inferior para cada nivel
    switch (validDifficulty) {
        case 1: min = 1; max = 9; break; // Nivel 1: siempre 1-9 + 1-9
        case 2: min = 10; max = 99; lowerMin = 1; lowerMax = 9; break; // L2 puede sumar con L1
        case 3: min = 100; max = 999; lowerMin = 10; lowerMax = 99; break; // L3 puede sumar con L2
        case 4: min = 1000; max = 9999; lowerMin = 100; lowerMax = 999; break; // L4 puede sumar con L3
        case 5: min = 10000; max = 99999; lowerMin = 1000; lowerMax = 9999; break; // L5 puede sumar con L4
        default: min = 1; max = 9; // Fallback
    }

    // Generar el primer número siempre dentro del rango principal del nivel
    let num1 = getRandomInt(min, max);
    let num2: number;

    // Decidir aleatoriamente si el segundo número también será del rango principal
    // o si será del rango inferior (si aplica)
    const useSameRange = validDifficulty === 1 || Math.random() < SAME_RANGE_PROBABILITY;

    if (useSameRange) {
        // Generar num2 también del rango principal
        num2 = getRandomInt(min, max);
        // Opcional: intentar evitar num1 === num2 para mayor variedad en este caso
        if (num1 === num2 && max > min) { // Solo reintentar si el rango tiene más de un número
             let attempts = 0;
             while (num1 === num2 && attempts < 5) { // Evitar bucle infinito
                 num2 = getRandomInt(min, max);
                 attempts++;
             }
        }
    } else {
        // Generar num2 del rango inferior definido
        num2 = getRandomInt(lowerMin, lowerMax);
    }

    // Aleatoriamente intercambiar num1 y num2 para que el número más pequeño
    // no siempre aparezca en segundo lugar.
    if (Math.random() < 0.5) {
        [num1, num2] = [num2, num1];
    }

    // Asegurarse que la suma no sea negativa (aunque con números positivos es improbable)
    const correctAnswer = Math.max(0, num1 + num2);
...
// --- Settings Component --- (Sin cambios respecto a la versión anterior con Compensación)
export const Settings: React.FC = () => {
  const [settings, setSettings] = useState<Settings>(() => {
    const saved = localStorage.getItem('math_addition_settings');
  ...
  useEffect(() => {
    requestAnimationFrame(() => {
      localStorage.setItem('math_addition_settings', JSON.stringify(settings));
    });
  }, [settings]);
  ...
  return (
    <div className="max-w-lg mx-auto p-4">
      <h2 className="text-2xl font-bold mb-6 text-center">Addition Settings</h2>
      <div className="space-y-6">
         ...
         <div>
          ...
          <div className="mt-2"><p className="text-sm text-gray-600 dark:text-gray-400">
                {settings.difficulty === 1 && 'Sums like: 1-digit + 1-digit'}
                {settings.difficulty === 2 && 'Sums like: 2-digit + 2-digit (mostly), or 2-digit + 1-digit'}
                {settings.difficulty === 3 && 'Sums like: 3-digit + 3-digit (mostly), or 3-digit + 2-digit'}
                {settings.difficulty === 4 && 'Sums like: 4-digit + 4-digit (mostly), or 4-digit + 3-digit'}
                {settings.difficulty === 5 && 'Sums like: 5-digit + 5-digit (mostly), or 5-digit + 4-digit'}
          </p></div>
        </div>
        {/* Number of Problems */}
          ...
          <h3 className="text-lg font-medium mb-4">Difficulty Examples</h3>
            ...
            {[1, 3, 5].map((level) => (
              <div key={level} className={`bg-white dark:bg-gray-800 p-4 rounded-md shadow-sm ${ level === settings.difficulty ? 'ring-2 ring-indigo-500' : ''}`}>
                ...
                {(() => { const p1 = generateProblem(level); const p2 = generateProblem(level); // Generar ejemplos con la nueva lógica
                  return ( <>
...
// --- Exercise Component --- (El resto del componente permanece igual que la versión anterior completa)
export const Exercise: React.FC = () => {
  ...
  const emitProgress = useCallback((data: { correct: boolean; timeSpent: number; difficulty: number; attempts: number; revealed: boolean }) => {
    const event = new CustomEvent('operationProgress', { detail: { operationType: 'addition', ...data } });
  ...
  useEffect(() => { // Listener localStorage
    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === 'math_addition_settings' && event.newValue) {
        try {
  ...
  useEffect(() => { // Iniciar/Reiniciar Ejercicio
    console.log("Initializing/Restarting exercise with settings:", userSettings);
    const initialDifficulty = userSettings.difficulty;
    const generateCache = (difficulty: number, size: number) => Array(size).fill(null).map(() => generateProblem(difficulty)); // Usa la nueva generateProblem
  ...
  useEffect(() => { // Lógica Auto-Continuar
  ...
  const triggerContinuation = useCallback(() => {
    if (isAutoContinueEnabled && !isReviewMode) {
      setShowContinueButton(false); // Dejar que el useEffect maneje el timer
    } else {
      setShowContinueButton(true); // Mostrar botón manual
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isNaN(numericAnswer)) { setError('Please enter a valid number'); isSubmitting.current = false; inputRef.current?.focus(); inputRef.current?.select(); return; }
    if (numericAnswer < 0 && currentProblem.num1 >= 0 && currentProblem.num2 >= 0) { setError('Answer cannot be negative for this addition'); isSubmitting.current = false; inputRef.current?.focus(); inputRef.current?.select(); return; }
    ...
    if (isCorrect) {
        ...
        if (userSettings.adaptiveDifficulty) {
            ...
            if (newStreak >= CORRECT_STREAK_THRESHOLD && currentExerciseDifficulty < MAX_DIFFICULTY) {
                ...
                const newProblems = Array(cacheSize).fill(null).map(() => generateProblem(nextDifficulty)); // Usa la nueva generateProblem
  ...
  const handleContinue = useCallback(() => {
    ...
    if (nextProblemIndex < problemCache.length) {
      ...
      console.warn("Generating problem outside cache for index:", nextProblemIndex);
      nextProblem = generateProblem(currentExerciseDifficulty); // Usa la nueva generateProblem
    ...
    setProblemStartTime(Date.now());
    isSubmitting.current = false;
  }, [currentIndex, targetProblemCount, problemCache, currentExerciseDifficulty]); // generateProblem no necesita estar aquí si es pura
  ...
  const handleAutoContinueChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    else { setShowAutoContinueTooltip(false);
      if (autoContinueTimerRef.current) { clearTimeout(autoContinueTimerRef.current); autoContinueTimerRef.current = null; if (feedback && !isComplete) setShowContinueButton(true); }
    }
  };
  ...
  const handleEnterReview = () => { if (userAnswers.length === 0) return; setIsReviewMode(true); setReviewIndex(userAnswers.length - 1); if (autoContinueTimerRef.current) { clearTimeout(autoContinueTimerRef.current); autoContinueTimerRef.current = null; } setShowContinueButton(false); };
  const handleExitReview = () => { setIsReviewMode(false); if (feedback && isAutoContinueEnabled && !isComplete) triggerContinuation(); else if (feedback && !isAutoContinueEnabled && !isComplete) setShowContinueButton(true); };
  ...
  // --- Render Helper para Modo Revisión ---
  const renderReviewScreen = () => { // Sin cambios visuales, usa los datos existentes
      ...
      const renderReviewEquation = () => { /* ... (código idéntico al anterior) ... */
  ...
  if (isComplete) { // Pantalla de Resumen Final (sin cambios visuales, usa datos actualizados)
       ...
       const renderSummaryItem = (answer: UserAnswer, index: number) => { /* ... (código idéntico al anterior) ... */
           const { problem, userAnswer, isCorrect, wasRevealed, timeSpent, attemptsMade, difficultyLevel } = answer; const displayValue = wasRevealed ? problem.correctAnswer : (userAnswer ?? '?'); let valueClass = ""; let correctionSpan: React.ReactNode = null; let statusIcon: React.ReactNode = null;
        ...
        <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <div className="mb-8">
                 <div className="flex justify-between items-center mb-2 text-sm">
                     <button onClick={handleEnterReview} disabled={userAnswers.length === 0} className="p-1 text-gray-500 hover:text-indigo-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors" aria-label="Review Previous Problems" title="Review Previous Problems"> <Eye size={18} /> </button>
                     <span className="font-medium text-gray-700 dark:text-gray-300">Problem {currentIndex + 1} of {targetProblemCount}</span>
                     <div><span className="text-gray-600 dark:text-gray-400 mr-3">Lvl: <span className="font-semibold text-indigo-700 dark:text-indigo-300">{currentExerciseDifficulty}</span></span> <span className="text-gray-600 dark:text-gray-400">Att: {currentAttempts} / {userSettings.maxAttempts === 0 ? '∞' : userSettings.maxAttempts}</span></div>
                 </div>
            ...
            <motion.div key={`problem-container-${currentIndex}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-xl shadow-lg mb-6 text-center border border-gray-200 dark:border-gray-700 relative">
                 <div className="text-3xl sm:text-4xl font-bold mb-6 text-gray-800 dark:text-gray-100 flex justify-center items-center space-x-3 sm:space-x-4 flex-wrap"><span>{currentProblem.num1}</span> <span className="text-indigo-600 dark:text-indigo-400">{currentProblem.operator}</span> <span>{currentProblem.num2}</span> <span>=</span> <span className="text-indigo-600 dark:text-indigo-400">?</span></div>
