...
export const Exercise: React.FC = () => {
  ...
  useEffect(() => {
    const newProblem = generateProblem(settings.difficulty, settings.includeDivisibleOnly);
    setCurrentProblem(newProblem);
    startTime.current = Date.now();
  }, [settings.difficulty, settings.includeDivisibleOnly]);
  ...
  const handleContinue = () => {
    ...
    if (currentIndex < targetProblemCount - 1) {
      ...
      setCurrentProblem(generateProblem(settings.difficulty, settings.includeDivisibleOnly));
      setNumerator('');
      setDenominator('');
      setShowSteps(false);
  ...
  const calculateIntermediateResult = (problem: Problem) => {
    return problem.wholeNumber * problem.targetDenominator;
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const userAnswer = { numerator: numValue, denominator: denValue };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-lg mb-6">
        <div className="text-center mb-6">
          <h2 className="text-2xl font-bold mb-4 text-pink-600 dark:text-pink-400">
            {settings.language === 'english' ? 'Convert Whole Number to Fraction' : 'Convertir Número Entero a Fracción'}
          </h2>
          ...
          <div className="mb-6">
            ...
            <div className="text-4xl font-bold text-yellow-600 dark:text-yellow-400">
              {currentProblem.wholeNumber}
            </div>
            
            {currentProblem.targetDenominator !== 1 && (
              <p className="text-lg text-gray-600 dark:text-gray-400 mt-4">
                {settings.language === 'english' 
          ...
          {showSteps && (
            ...
            >
              {currentProblem.targetDenominator === 1 ? (
                <>
                  {/* Steps for whole number to fraction conversion */}
                  <div>
                    <p className="text-xl text-pink-600 dark:text-pink-400 mb-2">
                      {settings.language === 'english'
                        ? `Write the whole number ${currentProblem.wholeNumber} as numerator`
                        : `Escribe el número entero ${currentProblem.wholeNumber} como numerador`}
                    </p>
                    <div className="text-2xl font-bold flex justify-center space-x-4">
                      <div className="flex flex-col items-center">
                        <span className="text-yellow-600 dark:text-yellow-400">{currentProblem.wholeNumber}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    <p className="text-xl text-pink-600 dark:text-pink-400 mb-2">
                      {settings.language === 'english'
                        ? 'Place 1 as the denominator'
                        : 'Coloca 1 como denominador'}
                    </p>
                    <div className="text-2xl font-bold flex justify-center space-x-4">
                      <div className="flex flex-col items-center">
                ...
                <>
                  {/* Steps for whole number to fraction with specified denominator */}
                  <div>
                    <p className="text-xl text-pink-600 dark:text-pink-400 mb-2">
                      {settings.language === 'english'
                        ? `Multiply the whole number ${currentProblem.wholeNumber} by the denominator ${currentProblem.targetDenominator}`
                        : `Multiplica el número entero ${currentProblem.wholeNumber} por el denominador ${currentProblem.targetDenominator}`}
                    </p>
                    <div className="text-2xl font-bold flex items-center justify-center space-x-2">
                      ...
                      <span className="text-gray-600 dark:text-gray-400">=</span>
                      <span className="text-green-600 dark:text-green-400">{intermediateResult}</span>
                    </div>
                  </div>
                  ...
                  <div>
                    <p className="text-xl text-pink-600 dark:text-pink-400 mb-2">
                      {settings.language === 'english'
                        ? `Use ${intermediateResult} as the numerator and ${currentProblem.targetDenominator} as the denominator`
                        : `Usa ${intermediateResult} como numerador y ${currentProblem.targetDenominator} como denominador`}
                    </p>
                    <div className="text-2xl font-bold flex justify-center space-x-4">
                      <div className="flex flex-col items-center">
                        <span className="text-yellow-600 dark:text-yellow-400">{currentProblem.correctAnswer.numerator}</span>
